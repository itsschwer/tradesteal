...
execute unless block ~-1 ~-1 ~ minecraft:blast_furnace{CustomName: '{"text":"Take [1-10]"}'} run data merge entity @s {Tags:["sts_rejected"]}
execute unless block ~1 ~-1 ~ minecraft:blast_furnace{CustomName: '{"text":"Replace [1-8]"}'} run data merge entity @s {Tags:["sts_rejected"]}

# Validation fail feedback
execute if entity @e[type=minecraft:area_effect_cloud,tag=sts_anchor,distance=..1] run tellraw @a[distance=..5] ["",{"text":"["},{"text":"Tradesteal","color":"#C77AFF"},{"text":"] "},{"text":"A nearby structure already exists","color":"green"}]
...
# Validation success
execute unless entity @e[type=minecraft:area_effect_cloud,tag=sts_anchor,distance=..1] run execute if block ~ ~-1 ~ minecraft:crying_obsidian run execute if block ~-1 ~-1 ~ minecraft:blast_furnace{CustomName: '{"text":"Take [1-10]"}'} run execute if block ~1 ~-1 ~ minecraft:blast_furnace{CustomName: '{"text":"Replace [1-8]"}'} run function schwer_villager_merge:setup_structure
