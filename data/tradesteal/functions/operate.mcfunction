...
execute store result score targetIndex sts_score run data get entity @s Inventory[{Slot:-106b}].Count
...
data modify storage schwer:sts_store WorkingArray set from entity @e[limit=1,sort=nearest,distance=..5,type=villager,tag=sts_recipient] Offers.Recipes

# Cycle desired replace offer to slot 0
execute store result score arrayLength sts_score run data get storage schwer:sts_store WorkingArray
# 'Replace' is held in mainhand (refer to valid_player predicate)
execute store result score targetIndex sts_score run data get entity @s SelectedItem.Count
scoreboard players set currentIndex sts_score 1
function tradesteal:cycle_array_loop
...
data modify entity @e[limit=1,sort=nearest,distance=..5,type=villager,tag=sts_recipient] Offers.Recipes set from storage schwer:sts_store WorkingArray
